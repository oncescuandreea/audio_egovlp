...
def calculate_mAP(sim_mat, relevancy_matrix, extra=False, reduction="mean"):
    """
    ...
    if (number_rel_docs == 0).sum() != 0:
        warnings.warn(
            f"Warning. There are {(number_rel_docs==0).sum()} 0s in number_rel_docs"
        )
        number_rel_docs[np.where(number_rel_docs == 0)] = 1
