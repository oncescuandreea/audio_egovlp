...
import logging
from .utils import freeze_batch_norm_2d, sim_matrix
...
class CLAP(nn.Module):
    ...
    # @profile
    def forward(self, audio, text, device=None, return_embeds=True, do_aggregation=False):
        ...
        audio_feats_tot = []
        # audio_features_mlp_tot = []
        list_keys = ['mel_fusion', 'longer', 'waveform']
        if audio['mel_fusion'].dim() == 5:
            # logging.info("This is assuming we are evaluating on EgoMCQ")
            for idx in range(0, 5):
                new_data = {}
                for list_key in list_keys:
                    new_data[list_key] = audio[list_key][:,idx,:]
                audio_features = self.audio_projection(self.encode_audio(new_data, device=device)["embedding"])
                audio_features = F.normalize(audio_features, dim=-1)
                audio_feats_tot.append(audio_features.detach())
                # audio_features_mlp = self.audio_transform(audio_features)
                # audio_features_mlp_tot.append(audio_features_mlp.detach())
                
            audio_features = torch.cat(audio_feats_tot, dim=-2)
            # audio_features_mlp = torch.cat(audio_features_mlp_tot, dim=-2)
        else:
            # logging.info("Classic model")
        ...
        ###### Code taken from Frozen VVVV
        if return_embeds and not do_aggregation:
            return text_features, audio_features # from frozen but replaced with old code
            # return (
            #     audio_features,
            #     text_features,
            #     audio_features_mlp,
            ...
            # return text_features, audio_features, text_parent_embeddings, video_parent_embeddings, text_stacked_embeddings
            return text_features, audio_features, None, None, None
        elif not return_embeds and not do_aggregation:
            return sim_matrix(text_features, audio_features)
        else:
            raise NotImplementedError
