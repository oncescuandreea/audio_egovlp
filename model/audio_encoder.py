...
from model.cnns import Cnn14, ResNet38
        ...
        if config["audio_encoder_args"]["type"] == "cnn":
            ...
            if config["audio_encoder_args"]["pretrained"]:
                ...
                pretrained_cnn = torch.load(
                    "pretrained_models/audio_encoder/{}.pth".format(
                        config["audio_encoder_args"]["model"]
                    )
                )["model"]
                dict_new = self.audio_enc.state_dict().copy()
                trained_list = [
                    i
                    for i in pretrained_cnn.keys()
            ...
            if config["audio_encoder_args"]["pretrained"]:
                audio_ckpt = torch.load(
                    "pretrained_models/audio_encoder/HTSAT.ckpt", map_location="cpu"
                )["state_dict"]
