...
class TextAudVideoDataset(Dataset):
    def __init__(
        self,
        dataset_name,
        text_params,
...
def read_frames_cv2_egoclip(
    video_path_1, video_path_2, num_frames, sample, start_sec, end_sec, bound_sec
):
...
def read_frames_cv2_charades(
    video_path, num_frames, sample, start_sec=None, end_sec=None
):
    cap = cv2.VideoCapture(video_path)
    assert cap.isOpened()
    vlen = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
...
def read_frames_av(video_path, num_frames, sample="rand", fix_start=None):
    ...
    try:
        ...
        print(
            "{}: WEBM reader cannot open {}. Empty " "list returned.".format(
                type(exception).__name__, video_path
            )
        )
    vlen = len(frames)
    frame_idxs = sample_frames(num_frames, vlen, sample=sample, fix_start=fix_start)
