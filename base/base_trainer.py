...
class BaseTrainer:
    ...
    def __init__(
        self, model, loss, metrics, optimizer, config, writer=None, init_val=False
    ):
        self.config = config
                    ...
                    for subkey, subval in value.items():
                        for subsubkey, subsubval in subval.items():
                            for subsubsubkey, subsubsubval in subsubval.items():
                                log[
                                    f"val_{subkey}_{subsubkey}_{subsubsubkey}"
                                ] = subsubsubval
                else:
                    log[key] = value
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        redo_dp = False
        if not curr_state_dict_keys[0].startswith("module.") and load_state_dict_keys[
            0
        ].startswith("module."):
            undo_dp = True
        elif curr_state_dict_keys[0].startswith("module.") and not load_state_dict_keys[
            0
        ].startswith("module."):
            redo_dp = True
            undo_dp = False
...
class Multi_BaseTrainer_dist:
    ...
    def __init__(
        ...
        if model_v is not None:
            ...
            self.model_v.device = self.device
        else:
            self.model_v = model_v

        if model_tav is not None:
            self.model_tav = model_tav.to(self.device)
            self.model_tav.device = self.device
        else:
            self.model_tav = model_tav
        ...
        if len(device_ids) > 1:
            ...
            if not isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                try:
                    ...
                    if model_v is not None:
                        self.model_v = torch.nn.parallel.DistributedDataParallel(
                            model_v, device_ids=[args.gpu], find_unused_parameters=True
                        )
                        if model_tav is not None:
                            self.model_tav = torch.nn.parallel.DistributedDataParallel(
                                model_tav,
                                device_ids=[args.gpu],
        ...
        if self.monitor == "off":
            ...
            self.early_stop = cfg_trainer.get("early_stop", inf)
            if self.args.rank == 0:
                print(f"mnt_metric is {self.mnt_metric}")
                print(f"mnt_best is initially {self.mnt_best}")
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        checkpoint = torch.load(
            resume_path, map_location="cuda:{}".format(self.args.rank)
        )
        self.start_epoch = checkpoint["epoch"] + 1
        self.mnt_best = checkpoint["monitor_best"]
